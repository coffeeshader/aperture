#+title: Aperture Science System Configuration
#+description: My NixOS configuration, as a flake.
#+options: toc:nil num:nil ^:nil

My NixOS configuration, as a flake.

* Layout

#+begin_example
flake.nix          inputs and mkHost helper; one line per host
hosts/<name>/      hardware scan + per-host module selection
modules/           system modules (common, audio, syncthing, desktop, gaming, vr)
packages/          reusable flake packages; dev environments via nix shell aperture#<env>
home/<name>.nix    per-host home entry (stateVersion)
home/common/       home-manager config, one file per topic (git, shell, emacs, ...)
home/profiles/     opt-in bundles a host imports (desktop, gaming, modding, library)
#+end_example

* Hosts

- *glados* - x86_64 desktop running Hyprland (until Niri supports HDR) on nixos-unstable.
- *chell* - x86_64 laptop running Niri on nixos-unstable.

* Usage

Rebuild the system:

#+begin_src sh
nh os switch
#+end_src

Or without nh:

#+begin_src sh
nixos-rebuild switch --flake ~/aperture --elevate=run0
#+end_src

Format the Nix files:

#+begin_src sh
nix fmt
#+end_src

* License

[[file:LICENSE][MIT]]
